...
class Robot : public frc::TimedRobot {
  ...
  const units::kilogram_square_meter_t kArmMOI = 1.2_kg_sq_m; // Moment of inertia of the arm. Can be
   ...
   */
  frc::LinearSystem<2, 1, 1> m_armPlant = frc::SingleJointedArmSystem(
    frc::DCMotor::NEO(2),
    kArmMOI,
    kArmGearing);
  ...
  frc::LinearQuadraticRegulator<2, 1> m_controller{m_armPlant,
        {1.0 * 2.0 * wpi::math::pi / 360.0, 10.0 * 2.0 * wpi::math::pi / 360.0}, // qelms.
 ...
 public:
  ...
  void TeleopInit() {
    m_lastProfiledReference = {units::radian_t(m_encoder.GetDistance), units::radians_per_second_t(m_encoder.GetRate)};
  }
  ...
  void TeleopPeriodic() {
    ...
    m_lastProfiledReference = (frc::TrapezoidProfile<units::radian_t>(m_constraints, goal,
